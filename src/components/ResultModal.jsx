    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-5"
        ...
        <div className="modal-pop bg-white rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden">
          {/* Header banner */}
          <div className={`py-7 px-5 text-center ${won ? 'bg-gradient-to-r from-yellow-400 to-yellow-600' : 'bg-gradient-to-r from-gray-400 to-gray-600'}`}>
          ...
          <div className="p-7 text-center">
            {won ? (
              <>
                <p className="text-gray-600 text-sm mb-3">您透過以下好友的推薦獲得獎勵</p>
                <div className="inline-block bg-yellow-50 border-2 border-yellow-400 rounded-2xl px-7 py-4 mb-5">
                  <p className="text-2xl font-bold text-yellow-700">{result.label}</p>
                  <p className="text-xs text-yellow-600 mt-1">推薦碼：{result.code}</p>
                </div>
          ...
          <div className="px-7 pb-7">
            <button
              onClick={onClose}
              className={`w-full py-3.5 rounded-2xl font-bold text-lg transition-all active:scale-95 ${
                won
